...
class SearchSeriePageKeyedDataSource(var query: String): PageKeyedDataSource<Int, Result>() {
    ...
    override fun loadInitial(
        ...
        CoroutineScope(Dispatchers.IO).launch {
            when(val response = repository.searchSerie(FIRST_PAGE)) {
                is ResponseAPI.Success -> {
                    var data = response?.let {
                        it.data as SearchSeries
                    }
                    data.results = data.results.filter { it.originalLanguage.equals("pt") && it.originCountry.equals("BR") }
                    data.results.forEach {result ->
                        ...
                        result.backdropPath.let { string ->
                            ...
                            result.backdropPath = result.posterPath.toString()
                        }
                    }
